...
def insert_words(words):
    ...
    for word in words[1]:
        ...
        if word != '':
            try:
                # 执行sql语句
                cursor.execute("""INSERT INTO index_introduction(words) VALUES(%s)""", (word))
                # 提交到数据库执行
                db.commit()
            except:
                # print("error")
...
if __name__ == "__main__":
    ...
    # 使用 cursor() 方法创建一个游标对象 cursor
    cursor = db.cursor()
    sql = "drop table index_introduction"
    cursor.execute(sql)

    # try:
    #     sql = """Create table index_introduction(
    ...
    for item in result:
        ...
        if y % 500 == 0:
            print("目前文章数：",y)
    print("第一次遍历分词结束")
    # f = open(r'index_introduction.txt', 'a')
    #
    # f.write('\n'.join(insert_w))
    #
    # f.close()
